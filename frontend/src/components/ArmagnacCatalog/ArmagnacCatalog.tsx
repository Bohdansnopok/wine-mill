...
const ArmagnacCatalog: React.FC = () => {
  ...
  const [armagnacs, setArmagnacs] = React.useState<any[]>([]);

  const fetchArmagnacs = async () => {
    const res = await fetch("http://localhost:4000/armagnacs");
...
export default ArmagnacCatalog;
